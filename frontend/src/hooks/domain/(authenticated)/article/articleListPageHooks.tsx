...
import { getNumberQueryParam } from '@/lib/getNumberQueryParam';
...
const LIST_API_PATH: AdminApiGetPaths = '/articles' as const;
 ...
 * 検索フォームの遷移先URLに利用
 */
const frontPagePath = '/articles' as const;
...
const useCurrentUrlParamsQuery = () => {
  ...
  const currentQuery: QueryParams = {
    ...
    category: getNumberQueryParam(urlParams, 'category'),
    page,
    par_page: parPage,
  };
 ...
 */
export const useArticleListData = () => {
 ...
 */
export const useArticleSearchForm = () => {
  const { currentQuery } = useCurrentUrlParamsQuery();
