...
export const getMyPageDetailHandler = http.get<
  never,
  AdminApiPaths['/mypage/{mypage}']['get']['requestBody'],
  AdminApiPaths['/mypage/{mypage}']['get']['responses']['200']['content']['application/json']
>(path, (req) => {
  const { id } = req.params;
